...
<body
	style="width: 100%; background-image: url('${bgimage}'); background-repeat:inherit; background-size:cover;">
					...
					<c:forEach items="${statusList }" var="status">
						<div class="col-md-2">
							<ul class="nav nav-pills">
								<li><a
									href="<spring:url value="/${sessionScope.designation}/Voucher/View Vouchers/${status.statusName}"/>">
							...
							<thead>
								<tr>
									...
									<c:forEach items="${requestScope.vouchers}" var="voucher1">
										<c:if test="${(voucher1.status eq 'Pending') eq breakfor}">
											<th>Cancel</th>
									...
									<c:set value="true" var="breakfor"></c:set>
									<c:forEach items="${requestScope.vouchers}" var="voucher1">
										<c:if test="${(voucher1.status eq 'Rejected') eq breakfor}">
											<th>Update</th>
											<c:set value="false" var="breakfor"></c:set>
										</c:if>
									</c:forEach>
							...
							<tbody>
								<c:forEach items="${requestScope.vouchers}" var="vouchers">
									<tr>
										...
										<c:if test="${vouchers.status eq 'Rejected'}">
											<td><a
												href="<spring:url value="/${sessionScope.designation}/Voucher/update/${vouchers.voucherId}"></spring:url>">
													<button type="button" class="btn btn-primary btn-md">
														Update</button>
											</a></td>
										</c:if>
										<c:if test="${vouchers.status eq 'Pending'}">
											<td><spring:url
													value="/${sessionScope.designation}/Voucher/Cancelled/${vouchers.voucherId}"
													var="cancel"></spring:url> <a href="${cancel}">
													<button type="button" class="btn btn-primary btn-md">
														Cancel</button>
											</a></td>
										</c:if>
